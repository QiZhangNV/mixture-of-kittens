...
@dataclass(frozen=True, slots=True)
class _FC1Components:
    """Internal adapter for callers that still hold gate and up separately."""
...
class MoKWorkspace:
    ...
    all_gather_top_experts_buffer: (
        torch.Tensor
    )  # (ep_size, num_local_tokens, topk) int32
...
def validate_workspace_args(
    ...
    *,
    ...
    if (
        ...
        raise ValueError(
            "schedule_capacity_multiplier must be a positive finite number"
        )
...
def create_workspace(
    ...
    *,
    ...
    if not torch.all(
        gathered_shapes == local_shape
    ).item():  # .item() here is fine since this is one-time setup
        raise ValueError(
    ...
    def allocate_buffer(
        *shape: int, dtype: torch.dtype, zero: bool = False
    ...
    x_buffer, x_buffer_handle, x_buffer_ptrs = allocate_buffer(
        num_local_tokens, hidden_size, dtype=torch.bfloat16
    )
    combine_buffer, combine_buffer_handle, combine_buffer_ptrs = allocate_buffer(
        num_local_tokens * topk, hidden_size, dtype=torch.bfloat16
    )
    d_y_buffer, d_y_buffer_handle, d_y_buffer_ptrs = allocate_buffer(
        num_local_tokens, hidden_size, dtype=torch.bfloat16
    )
    d_x_routed_buffer, d_x_routed_buffer_handle, d_x_routed_buffer_ptrs = (
        allocate_buffer(num_local_tokens * topk, hidden_size, dtype=torch.bfloat16)
    )
    router_weight_buffer, router_weight_buffer_handle, router_weight_buffer_ptrs = (
        allocate_buffer(num_local_tokens, topk, dtype=torch.float32)
    )
    (
        d_router_weight_buffer,
        d_router_weight_buffer_handle,
        d_router_weight_buffer_ptrs,
    ) = allocate_buffer(num_local_tokens, topk, dtype=torch.float32)

    all_gather_top_experts_buffer, all_gather_top_experts_buffer_handle, _ = (
        allocate_buffer(ep_size, num_local_tokens, topk, dtype=torch.int32)
    )
    ...
    barrier_buffer, barrier_buffer_handle, barrier_buffer_ptrs = allocate_buffer(
        1, dtype=torch.int32, zero=True
    )
    ...
    workspace = MoKWorkspace(
        ...
        d_y_buffer_handle=d_y_buffer_handle,
        d_y_buffer_ptrs=d_y_buffer_ptrs,
        d_x_routed_buffer=d_x_routed_buffer,
        d_x_routed_buffer_handle=d_x_routed_buffer_handle,
...
def build_schedule(
    ...
    *,
    ...
    if (
        config.fwd_num_comm_sms >= device_properties.multi_processor_count
    ...
    if (
        type(config.all_gather_top_experts_chunk_bytes) is not int
        or config.all_gather_top_experts_chunk_bytes <= 0
        or config.all_gather_top_experts_chunk_bytes % 16 != 0
    ):
        raise ValueError(
            "all_gather_top_experts_chunk_bytes must be a positive multiple of 16"
        )
    if (
        config.all_gather_top_experts_chunk_bytes + 1024
        > device_properties.shared_memory_per_block_optin
    ):
        raise ValueError(
            "all_gather_top_experts_chunk_bytes exceeds device dynamic shared-memory capacity"
        )
    ...
    if route_buffer_bytes % config.all_gather_top_experts_chunk_bytes != 0:
        raise ValueError(
            "all_gather_top_experts_chunk_bytes must divide one rank's route-buffer bytes"
        )
...
def validate_inputs(
    ...
    if grad_output is not None:
        tensors.append(
            ("grad_output", grad_output, torch.bfloat16, expected_activation_shape)
        )
    ...
    for tensor_name, tensor, expected_dtype, expected_shape in tensors:
        if (
            ...
            raise ValueError(
                f"{tensor_name} must be contiguous {expected_dtype} on the workspace CUDA device"
            )
...
def forward(
    ...
    shared_fc1_weight: torch.Tensor,
    shared_fc2_weight: torch.Tensor,
    routed_fc1_weight: torch.Tensor
    | tuple[torch.Tensor, torch.Tensor]
    | SplitRoutedWeight,
    routed_fc2_weight: torch.Tensor
    | tuple[torch.Tensor, torch.Tensor]
    | SplitRoutedWeight,
    ...
    Inputs:
        ...
        shared_fc1_weight:  bfloat16 [2 * intermediate_size, hidden_size]
        shared_fc2_weight:  bfloat16 [hidden_size, intermediate_size]
        routed_fc1_weight:  bfloat16 [num_local_experts, 2 * intermediate_size, hidden_size] or MXFP8 representation
        routed_fc2_weight:  bfloat16 [num_local_experts, hidden_size, intermediate_size] or MXFP8 representation
    ...
    intermediate_size = shared_fc2_weight.shape[-1]
    if isinstance(shared_fc1_weight, _FC1Components):
        shared_gate_weights, shared_up_weights = (
            shared_fc1_weight.gate,
            shared_fc1_weight.up,
        )
    else:
        shared_gate_weights = shared_fc1_weight.narrow(0, 0, intermediate_size)
        shared_up_weights = shared_fc1_weight.narrow(
            0, intermediate_size, intermediate_size
        )
    shared_fc2_weights = shared_fc2_weight
    if isinstance(routed_fc1_weight, _FC1Components):
        routed_gate_weights, routed_up_weights = (
            routed_fc1_weight.gate,
            routed_fc1_weight.up,
        )
    else:
        routed_gate_weights = routed_up_weights = routed_fc1_weight
    routed_fc2_weights = routed_fc2_weight
    ...
    if split_weights != isinstance(
        ...
        raise TypeError(
            "routed FC1/FC2 weights must use the same storage representation"
        )
    ...
    if split_weights:
        ...
        if any(
            ...
            raise ValueError("split routed FC1/FC2 weights must use one precision")
...
def recompute_forward_context(
    ...
    x: torch.Tensor,
    shared_fc1_weight: torch.Tensor,
    routed_fc1_weight: torch.Tensor | tuple[torch.Tensor, torch.Tensor],
    swiglu_limit: float | None = None,
) -> MoKForwardContext:
    ...
    if isinstance(shared_fc1_weight, _FC1Components):
        ...
        intermediate_size = shared_fc1_weight.shape[0] // 2
        shared_gate_weights = shared_fc1_weight.narrow(0, 0, intermediate_size)
        shared_up_weights = shared_fc1_weight.narrow(
            0, intermediate_size, intermediate_size
        )
    ...
    if isinstance(routed_gate_weights, tuple) != isinstance(routed_up_weights, tuple):
        raise TypeError(
            "routed gate and up weights must use the same precision representation"
        )
    if isinstance(routed_gate_weights, tuple) and (
        len(routed_gate_weights) != 2 or len(routed_up_weights) != 2
    ):
...
def _backward_impl(
    ...
    shared_fc1_weight: torch.Tensor,
    shared_fc2_weight: torch.Tensor,
    routed_fc1_weight: torch.Tensor | tuple[torch.Tensor, ...] | SplitRoutedWeight,
    routed_fc2_weight: torch.Tensor | tuple[torch.Tensor, ...] | SplitRoutedWeight,
    swiglu_limit: float | None = None,
    main_grads: tuple[
        torch.Tensor,
        torch.Tensor,
    ...
    Inputs:
        ...
        shared_fc2_weights: bfloat16 [hidden_size, intermediate_size]
        ...
        routed_fc2_weights: bfloat16 [num_local_experts, hidden_size, intermediate_size] or MXFP8 tensor tuple
        swiglu_limit:        float | None
    ...
    Outputs:
        ...
        d_routed_fc2_weights: bfloat16 [num_local_experts, hidden_size, intermediate_size]
        d_shared_gate_weights: bfloat16 [intermediate_size, hidden_size]
        d_shared_up_weights:   bfloat16 [intermediate_size, hidden_size]
        d_shared_fc2_weights: bfloat16 [hidden_size, intermediate_size]
    """
    ...
    if main_grads is not None:
        (
            shared_fc1_main_grad,
            routed_fc1_main_grad,
            shared_fc2_main_grad,
            routed_fc2_main_grad,
        ) = main_grads
        if isinstance(shared_fc1_main_grad, _FC1Components):
            shared_gate_main_grad, shared_up_main_grad = (
                shared_fc1_main_grad.gate,
                shared_fc1_main_grad.up,
            )
        else:
            shared_gate_main_grad = shared_fc1_main_grad.narrow(0, 0, intermediate_size)
            shared_up_main_grad = shared_fc1_main_grad.narrow(
                0, intermediate_size, intermediate_size
            )
        if isinstance(routed_fc1_main_grad, _FC1Components):
            routed_gate_main_grad, routed_up_main_grad = (
                routed_fc1_main_grad.gate,
                routed_fc1_main_grad.up,
            )
        else:
            routed_gate_main_grad = routed_up_main_grad = routed_fc1_main_grad
        main_grads = (
            shared_gate_main_grad,
            routed_gate_main_grad,
            shared_up_main_grad,
            routed_up_main_grad,
            shared_fc2_main_grad,
            routed_fc2_main_grad,
        )
    if main_grad_storage_tables is not None:
        routed_fc1_main_grad_table, routed_fc2_main_grad_table = (
            main_grad_storage_tables
        )
        if isinstance(routed_fc1_main_grad_table, _FC1Components):
            routed_gate_main_grad_table, routed_up_main_grad_table = (
                routed_fc1_main_grad_table.gate,
                routed_fc1_main_grad_table.up,
            )
        else:
            routed_gate_main_grad_table = routed_up_main_grad_table = (
                routed_fc1_main_grad_table
            )
        main_grad_storage_tables = (
            routed_gate_main_grad_table,
            routed_up_main_grad_table,
            routed_fc2_main_grad_table,
        )
    ...
    if (main_grad_storage_tables is not None) != split_weights:
        raise ValueError(
            "split routed weights require two split main-grad storage tables"
        )
    routed_precision_is_mxfp8 = (
        routed_gate_weights.scale is not None
        if split_weights
...
def backward(
    ...
    """Runs the MoE backward pass with canonical FC1/FC2 weights and gradients.

    FC1 uses the native combined SwiGLU layout [..., 2 * I, H]; FC2 uses
    [..., H, I]. The split gate/up representation exists only below this
    functional boundary.
    """
    raw = _backward_impl(
        config,
    ...
    if main_grads is None:
        d_routed_fc1 = torch.cat((d_routed_gate, d_routed_up), dim=-2)
        d_shared_fc1 = torch.cat((d_shared_gate, d_shared_up), dim=-2)
    else:
        d_shared_fc1, d_routed_fc1, d_shared_fc2, d_routed_fc2 = main_grads
    return (
        d_x,
        d_router_weights,
        d_routed_fc1,
...
def _forward_gate_up(
    ...
    """Private compatibility adapter for pre-FC1/FC2 standalone tests."""
    return forward(
        config,
        workspace,
        schedule,
        x,
        router_weights,
        _FC1Components(shared_gate_weights, shared_up_weights),
        shared_down_weights,
        _FC1Components(routed_gate_weights, routed_up_weights),
        routed_down_weights,
        swiglu_limit=swiglu_limit,
    )


def _recompute_forward_context_gate_up(
    config,
    workspace,
    schedule,
    x,
    ...
    """Private compatibility adapter for pre-FC1 standalone tests."""
    return recompute_forward_context(
        config,
        workspace,
        schedule,
        x,
...
def _backward_gate_up(
    ...
    """Private compatibility adapter for pre-FC1/FC2 standalone tests."""
    canonical_main_grads = None
    if main_grads is not None:
        canonical_main_grads = (
            _FC1Components(main_grads[0], main_grads[2]),
            _FC1Components(main_grads[1], main_grads[3]),
            main_grads[4],
            main_grads[5],
        )
    canonical_main_grad_tables = None
    if main_grad_storage_tables is not None:
        canonical_main_grad_tables = (
            _FC1Components(main_grad_storage_tables[0], main_grad_storage_tables[1]),
            main_grad_storage_tables[2],
        )
    return _backward_impl(
        ...
        routed_down_weights,
        swiglu_limit=swiglu_limit,
        main_grads=canonical_main_grads,
        main_grad_storage_tables=canonical_main_grad_tables,
    )
